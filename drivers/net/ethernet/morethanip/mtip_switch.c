...
static inline int mt5pt_switch_wait_for_mdio(struct mt5pt_switch *mt5pt)
{
	...
	ret = readl_poll_timeout(mt5pt->regs + MDIO_CFG_STATUS, val,
				 !(val & MDIO_CFG_STATUS_BUSY),
				 10, MDIO_TIMEOUT);
	if (ret)
		return -ETIMEDOUT;
...
static int mt5pt_switch_read(struct mii_bus *bus, int phy_id, int phy_reg)
{
	...
	dev_dbg(mt5pt->dev, "phy[%d] reg%d=0x%x\n", phy_id, phy_reg, readl(mt5pt->regs + MDIO_DATA));
	return 0;
}
			      ...
			      int phy_reg, u16 phy_data)
{
	...
	return mt5pt_switch_wait_for_mdio(mt5pt);
}
...
static int mt5pt_switch_port_ioctl(struct net_device *dev,
...
static int mt5pt_switch_probe_dt(struct mt5pt_switch *mt5pt, struct device *dev)
{
	...
	for_each_child_of_node(node, port_node) {
		 ...
		 */
		ret = of_get_phy_mode(phy_node, &phy_mode);
		if (ret < 0) {
		...
		if (!ndev) {
			...
			for (i = 0; i < port_nr; i++)
				free_netdev(mt5pt->ports[port_nr]->ndev);
...
static int mt5pt_switch_probe(struct platform_device *pdev)
{
	...
	mt5pt->regs = devm_platform_ioremap_resource(pdev, 0);
	...
	if (!mt5pt->bus) {
		ret = -ENOMEM;
		dev_err_probe(dev, ret, "failed to alloc mii bus\n");
		goto err_clk;
	}

	mt5pt->clk = devm_clk_get(dev, "hclk");
	if (IS_ERR(mt5pt->clk)) {
		dev_err_probe(dev, PTR_ERR(mt5pt->clk), "failed to get device clock\n");
	...
	if (!dev->of_node) {
		ret = -EINVAL;
		dev_err_probe(dev, ret, "Missing DT node\n");
		goto err_clk;
	}
...
err_clk:
	clk_disable_unprepare(mt5pt->clk);
