 ...
 * Copyright (C) 2016-2022 Renesas Electronics Europe Ltd
 *
...
static int rzn1_miic_setup_ports(struct rzn1_miic *miic, struct device *dev)
{
	...
	for_each_child_of_node(node, port_node) {
		 ...
		 */
		parent_node = get_parent_node(phy_node);
		parent_dev = of_find_device_by_node(parent_node);

		if (parent_node && (parent_node != phy_node))
		 ...
		 */
		mode = of_get_phy_mode(phy_node, &port->phy_if);
		if (mode < 0) {
...
static int rzn1_miic_probe(struct platform_device *pdev)
{
	...
	miic->regs = devm_platform_get_and_ioremap_resource(pdev, 0, NULL);
	if (IS_ERR(miic->regs))
	...
	ret = rzn1_miic_setup_ports(miic, dev);
	if (ret)
		clk_disable_unprepare(miic->clk);
