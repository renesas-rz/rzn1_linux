...
static void rzn1_timer_writel(struct rzn1_timer *timer, u32 val, unsigned long rg)
{
...
static int __init rzn1_timer_init(struct device_node *np)
{
	...
	if (IS_ERR(clk)) {
		if (PTR_ERR(clk) == -EPROBE_DEFER)
			pr_err("Clock source requested probe defer\n");
		else
	...
	pr_warn("rzn1_timer probe sucessful\n");
	return 0;
...
static int __init rzn1_timer_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;

	return rzn1_timer_init(dev->of_node);
}

static const struct of_device_id rzn1_timer_of_table[] = {
	{ .compatible = "renesas,rzn1-timer", },
	{ /* sentinel */ }
};

static struct platform_driver rzn1_timer_device_driver = {
	.driver = {
		.name = "renesas,rzn1-timer",
		.of_match_table = of_match_ptr(rzn1_timer_of_table),
		.suppress_bind_attrs = true,
	},
};
builtin_platform_driver_probe(rzn1_timer_device_driver, rzn1_timer_probe);
