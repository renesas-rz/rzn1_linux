...
static int rzn1_qspi_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf, size_t len)
{
	int ret;
...
static int rzn1_qspi_write_reg(struct spi_nor *nor, u8 opcode, const u8 *buf, size_t len)
{
	struct rzn1_qspi_slave *s = nor->priv;
	struct rzn1_qspi *q = s->host;
	int ret;
...
static int rzn1_qspi_prep(struct spi_nor *nor)
{
	struct rzn1_qspi_slave *s = nor->priv;
	struct rzn1_qspi *q = s->host;
...
static void rzn1_qspi_unprep(struct spi_nor *nor)
{
}
...
static const struct spi_nor_controller_ops nor_callbacks = {
...
static int rzn1_qspi_probe(struct platform_device *pdev)
{
	...
	/* find the clocks */
	q->clk_fw = devm_clk_get(dev, "hclk");
	if (IS_ERR(q->clk_fw)) {
	...
	for_each_available_child_of_node(dev->of_node, np) {
		...
		q->slave[i].nor.controller_ops = &nor_callbacks;
		...
		if (s->mtd->size > 0x1000000 && s->nor.addr_width == 4) {
			...
			/* Send BRWR command */
			s->nor.controller_ops->write_reg(&s->nor, 0x17, &extadd, 1);

			/* for everyone but spansion, send 0xb7 command */
			s->nor.controller_ops->write_reg(&s->nor, SPINOR_OP_EN4B, NULL, 0);
