...
/* "mbox_" + max length of event ID + null-terminator */
#define MBOX_DEV_FILE_SIZE	(5 + 10 + 1)
#define MBOX_DEVS_MAX		5	/* Number of char devices */

/* number of fifo elements per device for receiving messages */
#define MBOX_FIFO_MAX		BIT(3)

/* default timeout for mailbox write */
#define MBOX_TIMEOUT_MS		500

/* not editable defines */
#define MBOX_IOCTL_NUM		0x1337	/* magic number */
/* number mailbox data entries on PL320 driver */
#define MBOX_DATA_CNT		(7)
/* size of a single mailbox data entry on the PL320 driver */
#define MBOX_DATA_SIZE		(sizeof(uint32_t))
...
/*
 * get the version of the mailbox driver as 32 bit value
 * (higher 16 bit: major, lower 16 bit: minor)
 */
